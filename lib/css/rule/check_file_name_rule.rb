  ...
  module CSS
    module Rule
      ...
      class FileNameChecker
        ...
        def check_ad( name )
          [LogEntry.new('路径和文件名中不应该出现ad', :error)] if name =~ ADVERTISMENT
        end

        def check_underscore( name )
          [LogEntry.new('文件名中单词的分隔符应该使用中横线“-”', :error)] if File.basename(name) =~ /_/
        end

        def check_seperator( name )
          [LogEntry.new('文件夹只有需要版本区分时才可用中横线分隔，如fdev-v3', :error)] if File.dirname(name) =~ /-(?!v?[\d.])/
        end
